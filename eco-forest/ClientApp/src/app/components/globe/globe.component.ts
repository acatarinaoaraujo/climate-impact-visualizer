...
import { scaleSequentialSqrt } from 'd3-scale';  // <-- Import d3-scale here
import { interpolateYlOrRd } from 'd3-scale-chromatic';  // <-- Import d3-scale-chromatic here
...
@Component({
  ...
  imports: [HttpClientModule, CommonModule],  // <-- Add HttpClientModule and CommonModule here
})
export class GlobeComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    const colorScale = scaleSequentialSqrt(interpolateYlOrRd);  // <-- Use the imported functions here

    const getVal = (feat: any) => feat.properties.GDP_MD_EST / Math.max(1e5, feat.properties.POP_EST);

    // Fetch GeoJSON using HttpClient
